...
def check_mountpoints(name: str, sock: socket.socket) -> None:
    ...
    while not run_event.is_set():
        ...
        mount_map = {mp[0]: mp[2] for mp in active_mountpoints}
...
def handle_events(name: str, mgmt_sock: socket.socket) -> None:
    ...
    sender.setsockopt(zmq.SNDHWM, config.ZMQ_HWM)  # High water mark for send buffer
    ...
    sender_t = threading.Thread(target=sender_thread_func, daemon=True)
    sender_t.start()

    while not run_event.is_set():

        events = selector.select(timeout=0.01)  # 10ms timeout for low-latency data forwarding
        ...
        for key, _ in events:
            ...
            if conn == mgmt_sock:
                ...
                if data:
                    try:
                        mount = connections[fd].name
                        # Try direct send first for low latency
                        try:
                            sender.send_pyobj((fd, data), flags=zmq.NOBLOCK)
                            try:
                                BYTES_TRANSFERRED.labels(mountpoint=mount).inc(len(data))
                            ...
                            # ZMQ buffer full, queue for later
                            try:
    ...
    while not run_event.is_set():
        try:
            with socket.create_connection((config.MQTT_HOST, config.MQTT_PORT), timeout=5):
                logger.info(
                    f"Successfully connected to MQTT broker at {config.MQTT_HOST}:{config.MQTT_PORT}"
                )
                break
        except OSError as e:
            logger.error(
                f"Failed to connect to MQTT broker at {config.MQTT_HOST}:{config.MQTT_PORT}: {e}"
            )
